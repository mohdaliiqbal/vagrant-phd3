...
for i in $(eval echo {1..$NUMBER_OF_CLUSTER_NODES}); do 
   ...
   echo $HOST_IP phd$i.localdomain phd$i >> /etc/hosts 
   
   #this mess is to avoid the host authenticity prompt when passwordless ssh connects to a host. making ambari happy.
   ssh-keygen -R phd$i.localdomain
   ssh-keygen -R $HOST_IP
   ssh-keygen -R phd$i.localdomain,$HOST_IP
   ssh-keyscan -H phd$i.localdomain,$HOST_IP >> ~/.ssh/known_hosts
   ssh-keyscan -H $HOST_IP >> ~/.ssh/known_hosts
   ssh-keyscan -H phd$i.localdomain >> ~/.ssh/known_hosts
